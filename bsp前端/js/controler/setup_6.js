/**
 * @author wl
 * @datetime 2012-5-18
 * @description setup_6.js
 */

var queryUrl='';
var cmb = new ComboxOperate();
var queryTest='';

var setInfoMag =(function(){
	var it={},queryUrl='',params={};

	it.myQuery = function(){
		params = Ab.urlDecode(Ab.serializeForm('form_setup_6'));
		//queryTest='major/query/null/null/null/';
		queryUrl='coursestandard/list/'
				+(params.form_status==''?'null':params.form_status)+"/"
				+(params.form_stdCourseCode==''?'null':params.form_stdCourseCode)+"/"
				+(params.form_stdCourseName==''?'null':params.form_stdCourseName)+"/"
				+(params.form_subject=='全部'?'null':params.form_subject)+"/"

			;
		queryUrl=encodeURI(queryUrl);
	};
	it.init = function(){
		$('#stdCourseCode_add,#stdCourseName_add').live('blur',function(){
			it.checkForm(this);
		});
		// 绑定新增按钮
		$('#overlay_setup_6_add').click(function() {  //专业信息管理查询
			$.fn.nyroModalManual({
				width:600,
				title:"新增批次",
				content: it.popAdd()
			});
			//绑定保存按钮
			$('#InfoMagAddSave').unbind('click').bind('click',function(){
					it.save();
				it.getList(1);
			});

			return false;
		});
		//绑定查询按钮
		$("#form_query").click(function(){
			//记录查询条件
			it.myQuery();
			it.getList(1);
		});
		$("#form_query").click();
		$('#Btn_doRank').click(function(){
			$(".paixu_1,.paixu_2").slideToggle("fast");
			$('#form_setup_6').hide();
			it.doRank($('#form_subject_2').val());
		});

		// 绑定删除
		$('#overlay_setup_6_delete').click(function(){
			var ids=[];
			$('#js_course_tbody').find('input[type="checkbox"]').each(function(){
				if(this.checked==true){
					ids.push($(this).val());
				}
			});

			if(ids.length==0){
				$.fn.nyroModalManual({
					width:400,
					title:"系统提示",
					content: '<div class="overlay_delete_box"><h5 class="red"><img src="images/delete_ico.gif" />请选择专业</h5></div><div class="overlay_btn"><input type="button" id="win_sure" class="btn btn_green overlayClose" value="确定"/></div>'
				});

			}else{
				$.fn.nyroModalManual({
					width:400,
					title:"系统提示",
					content: '<div class="overlay_delete_box"><h5 class="red"><img src="images/delete_ico.gif" />删除后将无法恢复，确定要删除吗？</h5></div><div class="overlay_btn"><input type="button" id="Btn_deling" class="btn btn_green" value="确定"/> <input type="button" id="win_cancle" class="btn btn_gray overlayClose" value="取消"/></div>'
				});
				$('#Btn_deling').click(function(){
					it.del();
				})
			}

			return false;
		});
	}
	it.popAdd = function(){
			var s='';
			for(var k in G_data['subject']){
				s+='<option value="'+k+'">'+G_data['subject'][k]+'</option>';
			}
			var html='';
			  html+='<form id="win_form_subject_2" name="form_subject_2">';
			  html+='<p><label class="labeltitle"><i class="color_red">批次代码：</i></label><input type="text" id="stdCourseCode_add" class="half" name="majorCode" value=""  /></p>';
			  html+='<p><label class="labeltitle"><i class="color_red">批次名称：</i></label><input type="text" id="stdCourseName_add" class="half" name="majorName" value=""  /></p>';
			  html+='<p style="display:none"><label class="labeltitle">所属学科：</label>';
			  html+='<select name="subjectcode" id="subject_add">';
			  html+=s;
			  html+='</select>';
			  html+='</p>';
			  html+='<p>&nbsp;</p>';
			  html+='<div class="overlay_btn">';
			  html+='<input type="button" id="InfoMagAddSave" class="btn btn_green" value="保存"/>';
			  html+='<input type="button" class="btn btn_gray overlayClose" value="取消"/>';
			  html+='</div>';
			  html+='</form>';
		   return html;
	};
	it.getList = function(page){
		page = (page && page>0)?page=page:page=Math.abs(page);
		var url=G_ROOT + queryUrl + page+'/'+G_perpage;
		$.getJSON(url,function(data){
			if(data){
				var html='',list = data.list;
				$.each(list,function(k,v){
					html+='<tr>';
					if(user['uRole']==1) html+='<td class="first tc"><input type="checkbox" name="checkall" class="chk_stdCourse" code="'+v.CODE+'" value="'+v.ID+'" /></td>';
					html+='<td>'+v.CODE+'</td>';
					html+='<td>'+v.NAME+'</td>';
					html+='<td style="display:none">'+G_data['subject'][v.SUBJECT_CODE]+'</td>';
					html+='<td>'+v.CREATETIME+'</td>';
					html+='<td style="display:none" id="txt_'+v.ID+'">'+(v.STATUS==1?'<i class="color_gray">已启用</i>':'<i class="color_gray">已停用</i>')+'</td>';
					if(user['uRole']==1) html+='<td style="display:none"><a class="InfoMagstaus" href="javascript:" status="'+v.ID+'_'+v.STATUS+'">'+(v.STATUS==1?'停用':'启用')+'</a></td>';
					html+='</tr>';
				});
				html = throwNull(html);
				$('#js_course_tbody').html(html);
				$('#pageShow').html(gPage(data.rCount,page,'setInfoMag.getList'));

				$('.InfoMagstaus').click(function(){
					var obj=$(this),_tmp=($(this).attr('status')).split("_");
					var id=_tmp[0],status=_tmp[1]==1?2:1;
					var p={
						id : id,
						status : status,
						updater : user['uid']
					}
					var str = Ab.encode(p);
					$.ajax({
						type: "POST",
						data: str,
						dataType:"json",
						url:G_ROOT+'coursestandard/update',
						contentType:'application/json;charset=UTF-8',
						success: function(rs){
							if(rs.returnCode=='000000'){
								obj.attr('status',id+'_'+status).text(_tmp[1]==1?'启用':'停用');
								$('#txt_'+id).html(_tmp[1]==1?'<i class="color_gray">已停用</i>':'<i class="color_gray">已启用</i>');
							}
						}
					});

				})
			}
		});
	 }
	it.checkForm = function(obj){
		$(obj).nextAll().remove();
		$(obj).after('<cite></cite>');
		var val=$(obj).val(),maxlen=parseInt($(obj).attr('maxlength'));
			maxlen = maxlen >0 ? maxlen : 20;
		if(val.length<=0){
			$(obj).next().removeClass().addClass('error').text('不能为空');
			return false;
		}else if(val.length>maxlen){
			$(obj).next().removeClass().addClass('error').text('不能大于'+maxlen+'字');
			return false;
		}else{
			$(obj).next().removeClass().addClass('right').empty();
			return true;
		}
	};
	// 开始保存
	it.save = function (){
		//alert(1);
		if(!it.checkForm($('#stdCourseCode_add'))){
			$('#stdCourseCode_add').focus();
			return false;
		}
		if(!it.checkForm($('#stdCourseName_add'))){
			$('#stdCourseName_add').focus();
			return false;
		}

		if(!maskForm('win_form_subject_2')){
			return false;
		}

		//var majorCode=[],majorName=[],toSubject=[];
		var stdCourseCode_add = $("#stdCourseCode_add").val();
		var stdCourseName_add = $("#stdCourseName_add").val();
		var toSubject_add = $("#subject_add").val();
		params = {
				name : stdCourseName_add,
				code : stdCourseCode_add,
				creater : user['uid'],
				subjectCode : toSubject_add
		}
		var str = Ab.encode(params);
		if((/[\u4e00-\u9fa5]+/).test(stdCourseCode_add)){
			mAlert('层次代码不能为中文');
			return false;
		}
		var str = Ab.encode(params);

		$.ajax({
			type: "POST",
			data: str,
			dataType:"json",
			url:G_ROOT+'coursestandard/create',
			contentType:'application/json;charset=UTF-8',
			success: function(rs){
				if(rs.returnCode=='000000'){
					//var insertId=rs.entity;
					$.fn.nyroModalManual({
						width:400,
						title:"系统提示",
						content: '<div class="overlay_delete_box"><h5 class="red"><img src="images/delete_ico.gif" />保存完毕</h5></div><div class="overlay_btn"><input type="button" id="win_sure" class="btn btn_red overlayClose" value="确定" onclick="location=\'setup_6.html\'" /></div>'
					});
				}else if(rs.returnCode=='500004'){
					$.fn.nyroModalManual({
						width:400,
						title:"系统提示",
						content: '<div class="overlay_delete_box"><h5 class="red"><img src="images/delete_ico.gif" />'+rs.entity+'已存在</h5></div><div class="overlay_btn"><input type="button" id="win_sure" class="btn btn_red overlayClose" value="确定" onclick="location=\'setup_6.html\'" /></div>'
					});
				}
			}
		});
	}
	it.del=function(e){
		var ids=[],codeList=[];
		$.each($('#js_course_tbody').find('input[type="checkbox"]'),function(k,v){
			if($(v)[0].checked==true){
				$(v).parent().parent().remove();
				ids.push($(this).val());
				codeList.push($(this).attr('code'));
			}
		});
		var _tmp={
			idList : ids,
			userId : user['uid'],
			codeList:codeList
		};
		var str = Ab.encode(_tmp);
		$.ajax({
			type: "POST",
			data: str,
			dataType:"json",
			url:G_ROOT+'coursestandard/batchdel',
			contentType:'application/json;charset=UTF-8',
			success: function(rs){
				if(rs.returnCode=='000000'){
					setTimeout(function(){
						var insertId=rs.entity;
						$.fn.nyroModalManual({
							width:400,
							title:"系统提示",
							content: '<div class="overlay_delete_box"><h5 class="red"><img src="images/delete_ico.gif" />删除成功</h5></div><div class="overlay_btn"><input type="button" id="win_sure" class="btn btn_red overlayClose" value="确定" /></div>'
						});
						$("#chooseAll").children().attr({checked:false});
					},300);
				}else if(rs.returnCode=='600004'){
					mAlert(rs.entity);
				}else{
					setTimeout(function(){
						mAlert("删除失败");
					},300);
				}
				it.getList(G_currPage);
			}
		});

	};

	// 排序功能
	it.doRank = function(subjectcode){
		var queryUrl='coursestandard/list/null/null/null/'+subjectcode+'/';
			queryUrl=encodeURI(queryUrl);
		it.myQuery();
		var url=G_ROOT + queryUrl + '1/100000';
		$.getJSON(url,function(data){
			var list = data.list;
			if(list){
				var html='';
				$.each(list,function(k,v){
					html+='<tr>';
					html+='<td class="first">'+v.NUMS+'</td>';
					html+='<td>'+v.CODE+'</td>';
					html+='<td>'+v.NAME+'<input type="hidden" value="'+v.ID+'" /></td>';
					html+='</tr>';
				});
				html = throwNull(html);
				$('.paixu_2').find('.openable_tbody').html(html);
			}
			summary.tableRank({serial:true});
		});

		$('#Btn_doRankSave').click(function(){
			var ids=[];
			$('.paixu_2').find('input[type="hidden"]').each(function(){
				ids.push($(this).val());
			})
			var _tmp={
				idList	: ids,
				order	: true,
				userId	: user['uid']
			}
			var str = Ab.encode(_tmp);
			$.ajax({
				type: "POST",
				data: str,
				dataType:"json",
				url:G_ROOT+'coursestandard/batchdel',
				contentType:'application/json;charset=UTF-8',
				success: function(rs){
					if(rs.returnCode=='000000'){
						var insertId=rs.entity;
						$.fn.nyroModalManual({
							width:400,
							title:"系统提示",
							content: '<div class="overlay_delete_box"><h5 class="red"><img src="images/delete_ico.gif" />排序成功</h5></div><div class="overlay_btn"><input type="button" id="win_sure" class="btn btn_red overlayClose" onclick="javascript:void((function(){$(\'.paixu_1,.paixu_2\').slideToggle(\'fast\');})());" value="确定" /></div>'
						});
						$('#form_setup_6').show();
						it.getList(1);
					}
				}
			});
		})
	};

	return it;
})();



$(document).ready(function() {
	// 权限控制
	if(user['uRole']==1){
		$('#th_do,#do_area,#chooseAll').show();
	}else{
		$('#do_area').empty().addClass('th_empty');
	}
	cmb.subject();

	var html_subject='';
	for(var i in G_data['subject']){
		html_subject+='<option value="'+i+'">'+G_data['subject'][i]+'</option>'
	}
	$('#form_subject').html('<option>全部</option>'+html_subject);
	$('#form_subject_2').html(html_subject);
	$('#form_subject_2').change(function(){
		setInfoMag.doRank(this.value);
	});

	setInfoMag.init();

})



