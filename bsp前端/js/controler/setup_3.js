...
var setLevel=(function(){
	...
	it.myQuery = function(){
		params = Ab.urlDecode(Ab.serializeForm('levelSeach'));
		var queryTest='null/null/null/';
		//{code}/{name}/{province}/{pageno}/{pagesize}
		queryUrl='educationlevel/query/'
	...
	it.init =function(){
		//绑定查询按钮
		$("#Btn_levelSearch").click(function(){
			it.myQuery();
			it.getList(1);
		});
		$("#Btn_levelSearch").click();
		...
		$('#Btn_levelDel').click(function(){
			...
			if(ids.length==0){
				$.fn.nyroModalManual({
					width:400,
					title:"系统提示",
					content: '<div class="overlay_delete_box"><h5 class="red"><img src="images/delete_ico.gif" />请选择层次</h5></div><div class="overlay_btn"><input type="button" id="win_sure" class="btn btn_green overlayClose" value="确定"/></div>'
				});
		...
		$.getJSON(url,function(data){
			...
			if(list){
				...
				$.each(list,function(k,v){
					html+='<tr>';
					if(user['uRole']==1) html+='<td class="first tc"><input type="checkbox" name="checkall"  code="'+v.code+'" class="chk_level" value="'+v.id+'" /></td>';
					html+='<td>'+v.NUM+'</td>';
					html+='<td>'+v.code+'</td>';
					html+='<td>'+v.name+'</td>';
					html+='<td>'+v.createTime+'</td>';
					html+='<td id="txt_'+v.id+'">'+(v.status==1?'<i class="color_gray">已启用</i>':'<i class="color_gray">已停用</i>')+'</td>';
					if(user['uRole']==1) html+='<td><a class="levelstaus" href="javascript:" status="'+v.id+'##'+v.status+'">'+(v.status==1?'停用':'启用')+'</a></td>';
					html+='</tr>';
				});
				html = throwNull(html);
				$('#js_openable_tbody').html(html);
				$('#pageShow').html(gPage(data.rCount,page,'setLevel.getList'));
				$('.levelstaus').click(function(){
					...
					$.ajax({
						...
						data: str,
						dataType:"json",
						url:G_ROOT+'educationlevel/update',
	...
	it.del=function(){
		...
		$.ajax({
			...
			data: str,
			dataType:"json",
			url:G_ROOT+'educationlevel/batchdel',
			contentType:'application/json;charset=UTF-8',
			success: function(rs){
				if(rs.returnCode=='000000'){
					setTimeout(function(){
		...
		$.getJSON(url,function(data){
			...
			if(list){
				...
				$.each(list,function(k,v){
					...
					html+='<td>'+G_data['educationlevel'][v.code]+'<input type="hidden" value="'+v.id+'" /></td>';
					html+='</tr>';
				});
				html = throwNull(html);
