...
function getDetail(){
	...
	$.getJSON(url_status_people,function(data){
		...
		if(list){
			...
			//html='本批次总座位数<i class="color_red">'+list[0].ALLDATA+'</i>个，必修学生<i class="color_red">'+list[0].BIXIU+'</i>人，选修学生<i class="color_red">'+list[0].XUANXIU+'</i>人';
		...
		if(data){
			...
			$.each(list,function(k,v){
				...
				if(v.STATUS==1 || v.STATUS==3){
					...
					html+='<td name="option" class="qiang_seat"><input name="forseat" type="button" class="btn btn_red" value="抢座" /></td>'	;
				}
				html+='</tr>';
			});
			...
			if(flag_yzz){
				...
				// 更新状态
				var html_status	='已占座&nbsp;<input type="button" class="btn btn_green overlay_cancel_grab" onclick="disseat(\''+flag_yzz+'\')" value="退座（撤销选课）" /><br />';
				...
				$('#form_status').empty();
				if(currstatus=='公示中'||currstatus=='待选'){
					$('#choose_status').html(currstatus);
				}else{
			...
			$('.qiang_seat').find('.btn').live('click',function(){
				...
				$.getJSON(G_ROOT+'studentrobcourse/addcoursescarebuy/'+classInfo['planid']+'/'+studentid+'/2/'+PARAVALUE,function(data){
...
function getStatus(coursecode,termid){
	...
	$.getJSON(url_status+coursecode+'/'+user['uid']+'/'+termid,function(data){
		...
		if(list.length>0){
			var html='',v=list[0];
				html+='当前已有<i class="color_ora">'+v.ALLCOUNT+'</i>人占座，您是第<i class="color_ora">'+v.ROWP+'</i>名';
