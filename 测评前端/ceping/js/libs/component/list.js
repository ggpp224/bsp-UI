 ...
 */
 ...
 Ambow.define('Ambow.view.ListView',{
 	...
 	className:'default_table listtab',

 	tagName: 'table',
 	tplHd: '<thead class="gradual_bg"><tr><% _.each(list,function(item){ %> <th <% w=item.width; if(w){ if(w>1){%> width="<%= item.width %>" <%} else{ %> width="<%= item.width*100 %>" <% } } %> ><%= item.header %></th> <% }); %></tr></thead>',
 	...
 	renderList: function(con){
 		...
 		_.each(this.store.models ,function(m,idx){
 			...
 			var it = new Ambow.view.ListItem({model:m});
 			me.items.push(it);
 			me.addItem(it);
 		});
