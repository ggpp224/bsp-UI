...
$(document).ready(function() {
	...
	var tpl = new Ambow.XTemplate(
		...
		'<input type="button" value="继续下一题" id="js_btn_next" class="btn btn_submit" name="">',
	...
	$('#js_topicTypeSelect').html(Tiku.topicTypeSelect(7));
	...
	$("#js_btn_save,#js_btn_next").die().live('click',function(){
		...
		//$('#js_topicAnswerList').each(function(){
			...
			if(_content.length==0){
				$('#js_topicAnswerList').focus();
				$('#js_topicAnswerList').next().text('请输入题干内容');
				return false;
			}
			...
			success:function(data){
				if(data.returnCode=='000000'){
					...
					if(typeof(addType)=='undefined'){
						...
						if(_id == "js_btn_next"){
							window.location.href="exammanage_addAsk.shtml";
						}else{
							window.location.href = "exammanage.shtml";
							//window.location.href="exammanage_addAsk_edit.shtml?id="+data.entity+'&topicTypeId='+topicTypeId;
						}
					}else{
						...
						$.getJSON(url,function(data){
							...
							if(_id=='js_btn_next'){
								...
								input_select_change('examinationpaper_list_input_jianda.html');
							}else{
								$('#js_btn_back').click();
							}
						});
